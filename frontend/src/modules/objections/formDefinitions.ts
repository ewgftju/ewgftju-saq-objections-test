...
export function actionForm(
  ...
  let note = "Все отправки, подписи и регистрация выполняются только в демо.";
  switch (action) {
    case "screen":
      ...
      fields.push(
        input(
          ...
          c.assignee === "Не назначен" ? "Исполнитель ДВГА (демо)" : c.assignee,
        ),
    ...
    case "members":
      ...
      fields.push(
        ...
        check("shared", "Материалы направлены членам комиссии (демо)"),
      );
      note =
    ...
    case "hearing": {
      ...
      if ((values.mode || "hold") === "hold") {
        fields.push(
          ...
          check(
            "notified",
            "Извещение и предварительное решение направлены заявителю (демо)",
          ),
        );
        if (c.type === "control")
          fields.push(
            check(
              "issuerNotified",
              "Орган, принявший акт, также извещён (демо)",
            ),
          );
      } else
        fields.push(
    ...
    case "hearing-held":
      ...
      fields.push(
        ...
        check(
          "recorded",
          "Протокол и возможность ознакомления зарегистрированы (демо)",
        ),
      );
      note =
    ...
    case "vote":
      ...
      fields.push(
        ...
        input(
          ...
          "ДЕМОНСТРАЦИЯ: реквизит аудиозаписи",
        ),
        check(
          "recusalDecision",
          "Конфликты интересов проверены; решения по отводам оформлены (демо)",
        ),
      );
      note =
    ...
    case "sign":
      ...
      fields.push(
        ...c.members
          ...
          .map((member) =>
            check(
              `signed_${member.id}`,
              `${member.name}: подпись зафиксирована (демо)`,
            ),
          ),
        check("secretary", "Секретарь подписал протокол (демо)"),
        area(
          "reason",
      ...
      note =
        "Проект протокола — следующий рабочий день после заседания; подписи — в течение рабочего дня после получения проекта. В демо реальные ЭЦП не применяются.";
      break;
    case "forward":
      ...
      fields.push(
        ...
        check(
          "materials",
          "Комплект административного дела подготовлен (демо)",
        ),
      );
      ...
      fields.push(
        ...
        select(
          ...
          c.type === "notice"
            ...
            : [
                ["cabinet", "Кабинет SAQ (демо)"],
                ["sed", "СЭД (демо)"],
                ["post", "Почта (демо)"],
              ],
        ),
        input("receipt", "Квитанция отправки", "DEMO-" + c.id),
        ...
        check(
          "sent",
          "Мотивированный результат с порядком обжалования направлен заявителю (демо)",
        ),
        check("copy", "Копия передана органу, чей документ обжалуется (демо)"),
      );
      if (c.type === "notice")
        fields.push(
          check(
            "published",
            "Размещение решения на портале зарегистрировано (демо)",
          ),
        );
      note =
        c.type === "control"
          ? "Статья 100 АППК: письменный результат заявителю и копия органу. Вручение фиксируется отдельно после направления."
          : "Оформление — 2 рабочих дня с решения; публикация закупочного решения — 1 рабочий день после направления. Для уведомления формируется демонстрационное заключение по структуре приложения 7. Вручение фиксируется отдельно.";
      break;
    case "receipt":
    ...
    case "execute":
      ...
      fields.push(
        ...
        check(
          "checked",
          "Решение и оставшиеся пункты учтены в исходном деле (демо)",
        ),
      );
      note =
    ...
    case "supplement":
      ...
      fields.push(
        ...
        check("notified", "Извещение о продлении зарегистрировано (демо)"),
      );
      note =
    ...
    case "pause":
      ...
      fields.push(
        ...
        check(
          "notified",
          "Приостановление оформлено, извещение зарегистрировано (демо)",
        ),
      );
      note =
    ...
    case "refuse":
      ...
      fields.push(
        ...
        check(
          "notice",
          "Письменное извещение заявителю зарегистрировано (демо)",
        ),
      );
      note =
        action === "refuse"
